...
##*Introduction*
Public offenses can be defined as an act that deviates society's general ideas of normal social behavior and moral values. Different types of public crime can happen anywhere in the city. Some borough has more crimes, others has less. By identifying the hotspots from the past crime reports, it will bring caution to tourists and locals to be aware of a certain area during certain times. 

This project aims to explore and solve:

*Has the public crime activity in New York City increased or decreased at the end of 2017?

*Which borough in New York City has the most public crime complaints?
...
The dataset of daily average temperature and dataset of the day's precipitation greater than 3 inches from 2010 to 2017 in New York City is obtained from [Weather Underground](https://www.wunderground.com/). 
...
According to the NY Police Department post on the [The New York Job Source](http://www.nyjobsource.com/nypd.html), the NYPD shifts are divided by three 8-hours and 35 minute shifts: 11:15 PM to 7:50 AM, 7:05 AM to 3:40 PM, and 3:00 PM to 11:35 PM. But to simplify they call the shifts: (12 to 8), (8 to 4), and (4 to 12). I will be creating 3 time periods of the day corresponding to the police shifts to test which time period of the day has the most crime.


The dataframe only included the reported complaints for public offenses. The public premises include: "bus stop", "open areas (open lots)", "park/playground", "public buildings", "street", "transit (bus)", and "transit (subway)".  The public crimes includes: "arson", "assault and related offenses", "dangerous weapons", "felony assault", "harassment", "kidnapping", "rape", "robbery", and "sex crimes". The complaint reported for public crimes are 11.8% of all reported crimes between 2010 to 2017.

In the world of Criminal Justice, violations are considered to be the minor offenses. Violations can be punishable by a fine and will not result in any jail or prison time. Misdemeanor offenses are more serious than violation offenses. Misdemeanor can result up to one year in jail. Felonies are the most serious offense out of the three. Felonies are separated by letter (Class A - Class E). Class A felonies are the most serious and class E is the least. The punishments are sorted by class (A: up to lifetime in prison, B:25 years+, C: 10 - 25 years, D: 5 - 10 years, and E: 1 - 5 years). [Legal Dictionary](https://legaldictionary.net/felony/)

##*Data Limitations*
There isn't a specific whole area weather temperature for the entire New York City that includes all five boroughs on the historical data on the Weather Underground website. I took the average temperature of the most centered borough (Manhattan). 
...
There are limited data on the suspect's age, race, and sex because there might be a case where the suspect was never caught. Additionally, there are limited data on the victim's age, race and sex because of the protection of personal information.
...
Analysis of Violent or Potentially Violent Crime Complaints
=====================================================================================
...
###The total public crime complaints per month between 2010 to 2017 line graph below was created to see if there is any major changes through each of the year.
...
There is a decreasing trend in public crime complaints every year between the month of November, December, and January. The crime complaints start to increase after January and reaches the peak around the month of June and July of each year and drop again after.
...
According to the bar graph above, it clearly shown that Staten Island has significantly fewer public crime complaints than the other four boroughs. This is because of the size of population in the boroughs. According to the website (city population), Staten Island only hold 5.6% of the New York City population. But Brooklyn (population size of 2648771) shows to have significantly more public crime complaints than other boroughs even when Queens (population size of 2358582) has similar population size. Population size are determined by [City Population](https://www.citypopulation.de/php/usa-newyorkcity.php)
![New York City Boroughs Population Estimate](Population.png)

####The number of public crime complaints per 100 people was calculated for each borough to see which borough has the most complaints.
```{r echo = TRUE}
...
The complaints per capita is found by dividing the total public crime complaints with the population size for each borough. From the year of 2010 to 2017, there is a public crime complaint rate of 7.5 per 100 people in Bronx, 5.4 per 100 people in Brooklyn, 6 per 100 people in Manhattan, 3.9 per 100 people in Queens, and 3.4 per 100 people in Staten Island. 

The number of public crime complaints per 100 people for each boroughs bar graph show that Bronx has the most public crime complaints while Brooklyn has almost twice the amount of population. Staten Island has the least amount of population size and least amount of public crime complaints.
...
The annual total public crime complaints line graph above shown that Brooklyn's public crime complaints has a more noticeable sight of decrease over the years than the other boroughs. 
...
To determine if the number of public crime complaints have increased or decreased for each borough between 2010 and 2017, the percentage changes in public crime complaints between 2010 and 2017 is calculated. 
...
####The number of public crime complaints monthly for the five boroughs line graph below was created to see if there are any major changes in public crime complaints for the boroughs. 
``` {r echo = FALSE}
ggplot(AllComplaint, aes(x = as.Date(Year_Month), y = (..count..))) +
  ...
  xlab("Year") +
  ylab("Number of Public Crime Complaints") +
  ggtitle("Total Public Crime Complaints Monthly for The Five Boroughs") +
  theme(plot.title = element_text(hjust = 0.5))
```

The monthly complaints for the five boroughs line graph above once again show that there is significantly fewer public crime complaints during the month of November, December, and January for each borough. And there are significantly more public crime complaints during the month of June, July, and August for each borough.
...
The annual total public crime complaints per offense line graph above shown that harassment and sex crimes had clearly increased. The possession of dangerous weapons and robbery had clearly decreased, but other offenses are hard to tell.
...
The percentage changes between 2010 and 2017 are once again calculated by using the formula above. It helps to identify whether the offenses have decreased or increased in percentage. The percentage change in public crime complaints in 2017 from 2010: arson has decreased by 46.4%, assault & related offenses has decreased by 3.2%, possession of dangerous weapons has decreased by 46.7%, felony assault has decreased by 10.1%, harassment has increased by 3.8%, kidnapping has decreased by 31.3%, rape is unchanged, robbery has decreased by 38.6%, and sex crimes has increased by 25.6%.

###Exploring into the premises' description
####The bar graphs below are created to visualize the annual number of public crime complaints in each premises and the percentage of the total public crime complaints annually in each premises. 
``` {r echo = FALSE}
ggplot(AllComplaint) +
...
The majority of the total public crime complaints are from the street. Over the years, the percentage of street complaints has decreased while the percentage of transit - NYC subway has increased. 
...
####The bar graph below are created to visualize the suspect's race
```{r echo = FALSE}
ggplot(data = AllComplaint, aes(AllComplaint$Suspect_Race)) + 
  ...
  coord_flip()
```
The majority of the suspects that were reported were Black.

####The bar graphs below are created to visualize the suspect's race along with the boroughs
```{r echo = FALSE}
ggplot(AllComplaint) +
  ...
  coord_flip()
```
By looking more indepth on the suspect's race, I made a bar graph that shows the suspect's race along with each borough. There seems to be an interesting facts that Brooklyn has more Black and White suspects than any other boroughs and Queens has more Asian/Pacific Islander suspects than any other boroughs.
...
ggplot(data = AllComplaint, aes(AllComplaint$Suspect_Sex)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5))
```
The majority of the suspects that were reported were males. There are four times more males suspects than female suspects.
...
ggplot(data = AllComplaint, aes(AllComplaint$Victim_Race)) + 
  ...
  coord_flip()
```
Blacks are also the majority of the victim's race.
...
ggplot(AllComplaint) +
  ...
  coord_flip()
```
By looking more indepth on the victim's race, I made a bar graph that shows the victim's race along with each borough. There seems to be an interesting facts that Brooklyn also has more Black victims than any other boroughs and Queens has more Asian/Pacific Islander victims than any other boroughs.
...
By comparing the temperature with the total public crime complaints. We see a strong correlation between the daily average temperature and total crime complaint over the year of 2010 to 2017. As the temperature decreases, the public crime decreases as well and as the temperature increases, the public crime complaints increase as well.
...
From the nrow function, Shift 3 (hour 16:00 - 24:00) was found to have the most public crime complaints. Shift 1 has 118733 complaints, shift 2 has 145766 complaints, and shift 3 has 197408 complaints.

####The 3 bar graphs below is created to show the number of public crime complaints in each borough by shift and the stacked picture of the 3 shifts
```{r echo = FALSE}
ggplot((Shift1)) + 
...
Graphs shows that Shift1 has the least amount of crime complaints, followed by Shift2 and Shift3 has the most amount of crime complaints. There are fewer public crime complaints in Bronx than Manhattan and about the same number of public crime complaints in Queens during Shift1. But Bronx surpasses Manhattan and Queens number of public crime complaints during Shift2 and Shift3.
...
the stacked line graph above shows that in every year during the month of June, July, and August, the number of crime complaints in shift1 spiked up while shift2 decreases. This could be effect of summer time, the average temperature is warmer and students are in summer vacations.


Loading the library for plotting heatmap
```{r message = FALSE}
library(ggplot2)
library(ggthemes)
library(viridis)
...
##Exploring into the heatmap for NYC and each borough
...
nyc <- ggmap(nyc_map1)
nyc + stat_density2d(data=AllComplaint, aes(x = Lon, y = Lat, fill = ..nlevel..),alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
  ggtitle("New York City Heatmap") +
  theme(plot.title = element_text(hjust = 0.5))
```
The hotspots for crime complaints in New York City are in lower Manhattan, upper Manhattan, and lower left of Bronx.
...
man + stat_density2d(data=Manhattan, aes(x = man_lon, y = man_lat, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
...
brook + stat_density2d(data=Brooklyn, aes(x = brook_lon, y = brook_lat, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
  ggtitle("Brooklyn Heatmap") +
  theme(plot.title = element_text(hjust = 0.5))
```
The hotspots for crime complaints in Brooklyn are everywhere in the middle of Brooklyn Height, Flatbush Ditmas Park and Highlands Park.
...
bronx + stat_density_2d(data=Bronx, aes(x = bronx_lon, y = bronx_lat, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
  ggtitle("Bronx Heatmap") +
  theme(plot.title = element_text(hjust = 0.5))
```
The hotspots for crime complaints in Bronx are Fordham Heights, Morrisania, Mott Haven, and Soundview.
...
queens + stat_density2d(data=Queens, aes(x = queens_lon, y = queens_lat, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
...
staten + stat_density2d(data=StatenIsland, aes(x = staten_lon, y = staten_lat, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
...
##Exploring into the heatmaps for each borough by shifts
...
man + stat_density2d(data = Manhattan_Shift1, aes(x = Manhattan_lon_Shift1, y = Manhattan_lat_Shift1, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
  ggtitle("Manhattan Shift 1 Heatmap") +
  theme(plot.title = element_text(hjust = 0.5))

man + stat_density2d(data = Manhattan_Shift2, aes(x = Manhattan_lon_Shift2, y = Manhattan_lat_Shift2, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
  ggtitle("Manhattan Shift 2 Heatmap") +
  theme(plot.title = element_text(hjust = 0.5))

man + stat_density2d(data = Manhattan_Shift3, aes(x = Manhattan_lon_Shift3, y = Manhattan_lat_Shift3, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
...
Shift3 Manhattan hotspots: Between Hell's Kitchen and Midtown, and between Harlem and East Harlem.
...
brook + stat_density2d(data=Brooklyn_Shift1, aes(x = Brooklyn_lon_Shift1, y = Brooklyn_lat_Shift1, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
  ggtitle("Brooklyn Shift 1 Heatmap") +
  theme(plot.title = element_text(hjust = 0.5))

brook + stat_density2d(data=Brooklyn_Shift2, aes(x = Brooklyn_lon_Shift2, y = Brooklyn_lat_Shift2, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
  ggtitle("Brooklyn Shift 2 Heatmap") +
  theme(plot.title = element_text(hjust = 0.5))

brook + stat_density2d(data=Brooklyn_Shift3, aes(x = Brooklyn_lon_Shift3, y = Brooklyn_lat_Shift3, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
...
bronx + stat_density2d(data=Bronx_Shift1, aes(x = Bronx_lon_Shift1, y = Bronx_lat_Shift1, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
  ggtitle("Bronx Shift 1 Heatmap") +
  theme(plot.title = element_text(hjust = 0.5))

bronx + stat_density2d(data=Bronx_Shift2, aes(x = Bronx_lon_Shift2, y = Bronx_lat_Shift2, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
  ggtitle("Bronx Shift 2 Heatmap") +
  theme(plot.title = element_text(hjust = 0.5))

bronx + stat_density2d(data=Bronx_Shift3, aes(x = Bronx_lon_Shift3, y = Bronx_lat_Shift3, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
  ggtitle("Bronx Shift 3 Heatmap") +
  theme(plot.title = element_text(hjust = 0.5))
```

Shift1 Bronx hotspots: Morris Heights, Highbridge, Mott Haven, Morrisania and Soundview.

Shift2 Bronx hotspots: Morris Heights, Highbridge, Mott Haven, Morrisania and Soundview.
...
queens + stat_density2d(data= Queens_Shift1, aes(x = Queens_lon_Shift1, y = Queens_lat_Shift1, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
  ggtitle("Queens Shift 1 Heatmap") +
  theme(plot.title = element_text(hjust = 0.5))

queens + stat_density2d(data= Queens_Shift2, aes(x = Queens_lon_Shift2, y = Queens_lat_Shift2, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
  ggtitle("Queens Shift 2 Heatmap") +
  theme(plot.title = element_text(hjust = 0.5))

queens + stat_density2d(data= Queens_Shift3, aes(x = Queens_lon_Shift3, y = Queens_lat_Shift3, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
...
staten + stat_density2d(data= StatenIsland_Shift1, aes(x = StatenIsland_lon_Shift1, y = StatenIsland_lat_Shift1, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
  ggtitle("Staten Island Shift 1 Heatmap") +
  theme(plot.title = element_text(hjust = 0.5))

staten + stat_density2d(data= StatenIsland_Shift2, aes(x = StatenIsland_lon_Shift2, y = StatenIsland_lat_Shift2, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
  ggtitle("Staten Island Shift 2 Heatmap") +
  theme(plot.title = element_text(hjust = 0.5))

staten + stat_density2d(data= StatenIsland_Shift3, aes(x = StatenIsland_lon_Shift3, y = StatenIsland_lat_Shift3, fill = ..nlevel..), alpha = .1, geom = "polygon", size =0.5, bins=40) +
  scale_fill_gradient(name = "level", low =  "green", high = "red") +
...
Shift1 Staten Island hotspots: ST George, Tompkinsville, and Snug Harbor Cultural Center & Botanical Garden.

Shift2 Staten Island hotspots: ST George.

Shift3 Staten Island hotspots: ST George, Tompkinsville, and Snug Harbor Cultural Center & Botanical Garden.
...
####The str function is used to see the what format the variables are in
```{r echo = TRUE}
str(holiday)
```
The dataframe has 8 columns. The columns are: The complaint date, number of complaints of that day, average temperature of that day, if that day has precipitation greater than 3 inches, federal holidays descriptions, federal holidays, public schools closed descriptions, and public schools closed.
Precipitation greater than 3 inches has two factor levels, if the day has a precipitation greater than 3 inches then it will be "1 Yes" and if not then it will be "0 No".
...
```
By looking at the summary, I find that the average temperature is significant to number of complaints as the dependent variable ('***') according to the the significant codes. The summary shows a positive estimate, which means as the average temperature increase, the number of complaints increases as well. By using the confint function, the confident level of 95% forthe intercept and Average Temperature are stated.
...
confint(Precipitation)
```
By looking at the summary, the precipitation greater than 3 inches as the independent variable is significant to number of complaints as the dependent variable ('***') according to the significant codes. The summary shows a negative estimate, which means when there is a day that has precipitation greater than 3 inches, the number of complaints decreases. By using the confint function, the confident level of 95% for the intercept and precipitation greater than 3 inches are stated.
...
confint(Fed.hol.desc)
```
By looking at the summary, the federal holidays descriptions as the independent variables have different significant factor level to number of complaints as the dependent variable. Factor levels of "Christmas Day", "Martin Luther King Jr. Day", "New Year", "Thanksgiving Day", and "Presidents' Day" are significant with ('3stars'). Factor levels of "Christmas Day (lieu)", "Day after Thanksgiving", "Independence Day", and "Labor Day" are significant with ('2stars'). Factor level of "Lincoln's Birthday" are significant with ('1star'). Factor level of "New Year's Day (lieu)" are significant with ('.'). Factor levels of "Columbus Day", "Election Day", "Independence Day (lieu)", "Memorial Day", "Veterans Day", and "Veterans Day (lieu)" are not significant at all. The summary shows a positive estimate on "New Year's Day", "Independence Day", and "Labor Day" for the significant independent variables. The summary shows a negative estimate on "Christmas Day", "Christmas Day (lieu)", "Day after Thanksgiving", "Lincoln's Birthday", "Martin Luther King Jr. Day", "New Year's Day (lieu)", "Presidents' Day", and "Thanksgiving Day". By using the confint function, the confidence level of 95% for intercept and all the federal holiday descriptions are stated.

###Linear Regression Model with Number of complaints as dependent variable and Federal holiday as independent variable and looking at the summary
``` {r echo = FALSE}
Fed.hol <- lm(Number_of_Complaints~Federal_Holidays, data = holiday)
summary(Fed.hol)
confint(Fed.hol)
```
By looking at the summary, federal holiday as the independent variable is significant to number of complaints as the dependent variable ('***') according to the significant codes. The summary shows a negative estimate, which means when there is a federal holiday, the number of complaints decreases overall. By using the confint function, the confident level of 95% for the intercept and federal holidays are stated.

###Linear Regression Model with Number of complaints as dependent variable and Public school closed descriptions as independent variable and looking at the summary
``` {r echo = FALSE}
Publ.Sch.Clo.Des <- lm(Number_of_Complaints~Public_Schools_Closed_Description, data = holiday)
summary(Publ.Sch.Clo.Des)
confint(Publ.Sch.Clo.Des)
```
By looking at the summary, the public school closed descriptions as the independent variables have different significant factor level to number of complaints as the dependent variable. Factor levels of "Midwinter Recess", "Martin Luther King Jr. Day", "Thanksgiving Recess", "Winter Recess", and "Presidents' Day" are significant with ('3stars'). Factor levels of "Summer Vacation" and " Labor Day" are significant with ('1star'). Factor levels of "Election Day" and "Spring Recess" are significant with ('.'). Factor levels of "Clerical Day", "Columbus Day", "al-Adha", "al-Fitr", "Good Friday", "Memorial Day", "Rosh Hashanah", "Veterans Day", "Weekend", and "Yom Kippur" are not significant at all. The summary shows a positive estimate on "Labor Day" and "Summer Vacation" for the significant independent variables. The summary shows a negative estimate on "Election Day", "Midwinter Recess", "Martin Luther King Jr. Day", "Presidents' Day", "Spring Recess", "Thanksgiving Recess", and "Winter Recess" for the significant independent variables. By using the confint function, the confident level of 95% for the intercept and public school closed descriptions are stated.
...
confint(Publ.Sch.Clo)
```
By looking at the summary, public school closed as the independent variable is significant to number of complaints as the dependent variable ('.') according to the significant codes. The summary shows a positive estimate, which means when public school close, the number of complaints increases overall. By using the confint function, the confident level of 95% for the intercept and public schools closed are stated.
...
confint(holiday.model1)
```
Both average temperature of the day and precipitation greater than 3 inches are significant to the number of complaints ('***'). The multiple R-square increased from 0.482 to 0.5262.
...
confint(holiday.model2)
```
The independent variable of average temperature of the day, precipitation greater than 3 inches, and federal holiday are significant to the number of complaints ('***'). The multiple R-square increased from 0.5262 to 0.5304.
...
confint(holiday.model3)
```
When the public school closed independent variable is added to the linear regression model, the federal holidays significance decreases to ('**'). All other independent variables have the significant code ('3stars'). The multiple R-square increased from 0.5304 to 0.5488.
...
AIC(holiday.model3)
```
The root mean square error and AIC are the least on model3 and the multiple r-squared is the largest in model3.

### Splitting the data into training and test data
```{r echo = TRUE}
set.seed(314)
holiday.split <- sample.split(holiday$Number_of_Complaints, SplitRatio =  0.80)
holiday.train <- holiday[ holiday.split == TRUE, ]
nrow(holiday.train)
holiday.test <- holiday[ holiday.split == FALSE, ]
nrow(holiday.test)
```
Accoring to the Pareto principle, the dataset is splitted into training set and testing set. 80% is in the training set and 20% is in the testing set.

### Correlation covariance matrix
```{r echo = FALSE}
holiday2 <- holiday[, -c(1,5,7)]
pairs.panels(holiday2)
```
...
### Creating the model
```{r echo = TRUE}
holiday.model.train <- lm(Number_of_Complaints~ Average_Temperature+Precipitation_Greater_3inches+Federal_Holidays, data = holiday.train)
summary(holiday.model.train)
```
The train model is created

### Prediction
```{r echo = TRUE}
holiday.pred <- predict(holiday.model.train, holiday.test)
```
...
```
The test set number of complaints is in blue and the prediction set of complaints is in red. The prediction set of complaints has a very similar trend to the test set of complaints.
...
Manhattan During Shift 3 Hotspots: Between Hell's Kitchen and Midtown and between Harlem and 
East Harlem.

Brooklyn Hotspots: Everywhere in the middle of Brooklyn Height, Flatbush Ditmas Park and Highlands Park.
...
Bronx During Shift 1 Hotspots: Morris Heights, Highbridge, Mott Haven, Morrisania and Soundview.
Bronx During Shift 2 Hotspots: Morris Heights, Highbridge, Mott Haven, Morrisania and Soundview.
...
Staten Island During Shift 1 Hotspots: ST George, Tompkinsville, and Snug Harbor Cultural Center 
& Botanical Garden.
Staten Island During Shift 2 Hotspots: ST George.
Staten Island During Shift 3 Hotspots: ST George, Tompkinsville, and Snug Harbor Cultural Center 
& Botanical Garden.
```
...
Another further work that could be added can be a weighting scheme for how severe the crimes are and improving on the heatmap. Since crimes can be distinguished by their severe level, the more severe the crimes are then the more it should be weighted on the heatmap. One way to find how the crimes should be weighted, we can use the [Crime Severity Score data tool](https://www.ons.gov.uk/peoplepopulationandcommunity/crimeandjustice/datasets/crimeseverityscoredatatool) submitted by Mark Bangs. In his work, the list of weights provides the basis for deriving a severity score rather than comparing weights for individual offenses. Murder crimes are weighted 7973, which is the most severe crime out of the list.
